...
public class Store
{
    ...
    // This function will start a connection to a server.
    static public void start(String ip, int port) throws UnknownHostException, IOException
    {
        // Connect to a remote server on a given ip address and port.
        storeSocket = new Socket(ip, port);

        // Create input and output network buffers to communicate back and forth with the server.
        writer = new PrintWriter(storeSocket.getOutputStream(), true);
        reader = new BufferedReader(new InputStreamReader(storeSocket.getInputStream()));
    }

    // This function will close all our open utilities.
    ...
    /*
     * Main driver method.
     * @param args (program instructions)
     */
